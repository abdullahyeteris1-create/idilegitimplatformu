...
function MetricCard({ title, metric, days, field, tone }: { title: string; metric: DevelopmentMetric; days: DevelopmentReportDailyAverage[]; field: ChartField; tone: ChartTone }) {
  ...
  return <article className={`${styles.metricCard} ${styles[`metric${tone}`]}`}><div className={styles.metricIcon} aria-hidden="true">{tone === "blue" ? "↗" : tone === "green" ? "◎" : "✦"}</div><div className={styles.metricMain}><p>{title}</p><div className={styles.metricValues}><strong>{display.first}</strong><span>→</span><strong>{display.last}</strong></div><small>İlk gün ort.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Son gün ort.</small><div className={styles.metricDelta}>{display.delta}</div><em>{display.percent} gelişim</em></div><MiniSparkline days={days} field={field} tone={tone} /></article>;
}

function ProfileBar({ label, value, first, max, tone, suffix }: { label: string; value: number | null; first: number | null; max: number; tone: ChartTone; suffix: string }) {
  ...
  return <div className={styles.profileRow}><div className={styles.profileLabel}><span className={`${styles.profileIcon} ${styles[`tone${tone}`]}`} aria-hidden="true">{tone === "blue" ? "◌" : tone === "green" ? "◎" : "✦"}</span><strong>{label}</strong><small>{value === null ? "Son ölçüm yok" : `${valueOrDash(value, tone === "purple" ? 1 : 0)}${suffix}`}</small></div><div className={styles.profileScale}><div className={styles.track}><i className={styles[`track${tone}`]} style={{ width: `${current}%` }} />{marker !== null ? <b style={{ left: `${marker}%` }} aria-label={`Başlangıç: ${valueOrDash(first, tone === "purple" ? 1 : 0)}${suffix}`} /> : null}</div><div className={styles.scaleLabels}><span>0</span><span>{valueOrDash(max / 4, tone === "purple" ? 1 : 0)}</span><span>{valueOrDash(max / 2, tone === "purple" ? 1 : 0)}</span><span>{valueOrDash((max * 3) / 4, tone === "purple" ? 1 : 0)}</span><span>{valueOrDash(max, tone === "purple" ? 1 : 0)}</span></div></div>;<div className={styles.profileChange}>{first === null || value === null ? "—" : signed(value - first, suffix, tone === "purple" ? 1 : 0)}<small>{first === null ? "" : `Başlangıç: ${valueOrDash(first, tone === "purple" ? 1 : 0)}${suffix}`}</small></div></div>;
}
...
export default function DevelopmentReportClient({ report }: { report: DevelopmentReport }) {
  ...
  return <main className={styles.page}><div className={`${styles.toolbar} noPrint`}><a href="/ogretmen/idil-panel/ders-kayitlari">← Ders Kayıtları</a><div className={styles.toolbarActions}><button type="button" onClick={() => window.print()}>Yazdır / PDF Olarak Kaydet</button><button type="button" onClick={downloadPdf} disabled={isPdfPreparing}>{isPdfPreparing ? "PDF hazırlanıyor" : "↓ PDF İndir"}</button></div></div>{pdfError ? <p role="alert" className={`${styles.pdfError} noPrint`}>{pdfError}</p> : null}
    <article ref={reportRef} className={styles.report}><div className={styles.reportPageOne}>
      ...
      <section className={styles.section}><div className={styles.sectionHeading}><span className={styles.sectionMark} /><h2>PERFORMANS PROFİLİ</h2><small>Mevcut performans ve gelişim</small></div><div className={styles.profileCard}><ProfileBar label="Okuma Hızı" value={report.metrics.speed.last} first={report.metrics.speed.first} max={speedScale} tone="blue" suffix=" kelime/dk" /><ProfileBar label="Okuduğunu Anlama" value={report.metrics.comprehension.last} first={report.metrics.comprehension.first} max={100} tone="green" suffix="%" /><ProfileBar label="Odaklanma" value={report.metrics.focus.last === null ? null : report.metrics.focus.last / 10} first={report.metrics.focus.first === null ? null : report.metrics.focus.first / 10} max={10} tone="purple" suffix="/10" /></div></section>
