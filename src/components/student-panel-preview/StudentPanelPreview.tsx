...
import { getStudentPanelAchievements, getStudentPanelGameScores, getStudentPanelWeeklyProgress, type GameScoreSummary, type StudentAchievement, type WeeklyProgress } from "@/lib/student/studentPanelPhase2";
...
type StudentPanelPreviewProps = {
  ...
  showParagraphAnalyticsCard?: boolean;
...
const EXERCISE_ROUTE_BY_TYPE: Record<ExerciseType, string> = {
  ...
  "hecele-dinle-oku": "/egzersizler/hecele-dinle-oku",
  "emotional-reading": "/egzersizler/duygulu-okuma",
...
function LevelCard({
  ...
  recommendation?: StudentPanelRecommendation;
}) {
  ...
  return (
    ...
    >
      ...
      {!compact && recommendation ? <div className={styles.levelRecommendation} aria-labelledby="level-recommendation-title">
        <div className={styles.levelRecommendationHead}><span>✨ Bugün Sana Öneriyorum</span></div>
        <div className={styles.levelRecommendationBody}>
          <div className={styles.levelRecommendationThumb} aria-hidden="true">🎮</div>
          <div className={styles.levelRecommendationCopy}>
            <strong id="level-recommendation-title">{recommendation.title}</strong>
            <p>{recommendation.description}</p>
          </div>
          <Link href={recommendation.href} className={styles.levelRecommendationAction}>Oyna <Icon name="arrow"/></Link>
        </div>
      </div> : null}
    </section>
  );
...
function ParagraphAnalyticsCard() {
  return (
    <section className={styles.readingCard} aria-labelledby="paragraph-analytics-card-title" data-paragraph-analytics-card>
      <div className={styles.readingCardHead}>
        <h2 id="paragraph-analytics-card-title">Paragraf Analizim</h2>
        <p>Paragraf çalışmalarındaki gelişimini, güçlü yönlerini ve çalışma alanlarını gör.</p>
      </div>
      <Link href="/ogrenci/paragraf-analizim" className={styles.readingCardAction}>
        Analizimi Gör <Icon name="arrow" />
      </Link>
    </section>
  );
}
...
function getWeeklyComprehensionAverage(results: ExerciseResult[]): number | null {
  const end = Date.now();
  const start = end - 7 * 24 * 60 * 60 * 1000;
  const values = results.filter((result) => {
    const time = new Date(result.date).getTime();
    return result.exerciseType === "reading-comprehension" && Number.isFinite(time) && time >= start && time <= end;
  }).map((result) => getResultDetailNumber(result, "comprehensionScore") ?? result.successRate).filter((value) => Number.isFinite(value));
  return values.length ? clampPercentage(values.reduce((sum, value) => sum + value, 0) / values.length) : null;
}
...
function WeeklyProgressCard({ progress, results, loading }: { progress: WeeklyProgress; results: ExerciseResult[]; loading: boolean }) {
  const comprehensionAverage = getWeeklyComprehensionAverage(results);
  return <section className={styles.phase2Card} aria-labelledby="weekly-progress-title"><div className={styles.phase2Head}><h2 id="weekly-progress-title">📈 Bu Haftaki İlerlemen</h2><span>Son 7 gün</span></div>{loading ? <p className={styles.phase2Empty}>Veriler hazırlanıyor...</p> : <><div className={styles.phase2Metrics}><span><b>{progress.activeDays}</b> gün aktif</span><span><b>{progress.durationMinutes}</b> dk çalışma</span><span><b>{progress.completedCount}</b> çalışma</span>{progress.readingSpeedDelta !== null ? <span><b>{formatSignedDelta(progress.readingSpeedDelta)}</b><small>kelime/dk · haftalık değişim</small></span> : null}{comprehensionAverage !== null ? <span><b>%{comprehensionAverage}</b><small>ortalama anlama oranı</small></span> : null}</div><small className={styles.phase2SparklineLabel}>Günlük çalışma</small><div className={styles.phase2Sparkline} aria-label="Son 7 gün günlük çalışma yoğunluğu">{progress.trend.map((value, index) => <i key={`${index}-${value}`} style={{ height: `${Math.max(8, Math.min(100, value * 24 + 8))}%` }}/>)}</div></>}</section>;
}

function AchievementsCard({ achievements, loading }: { achievements: StudentAchievement[]; loading: boolean }) {
  return <section className={styles.phase2Card} aria-labelledby="achievements-title"><div className={styles.phase2Head}><h2 id="achievements-title">🏆 Son Başarıların</h2><span>En fazla 3</span></div>{loading ? <p className={styles.phase2Empty}>Başarıların hazırlanıyor...</p> : achievements.length ? <div className={styles.phase2AchievementGrid}>{achievements.map((item) => <article key={item.id} className={styles.phase2Achievement}><span aria-hidden="true">{item.icon}</span><div><strong>{item.title}</strong><small>{item.value}</small></div><em className={styles.phase2NewBadge}>Yeni</em></article>)}</div> : <p className={styles.phase2Empty}>Henüz doğrulanabilir yeni bir başarı yok.</p>}</section>;
}

function GameScoresCard({ scores }: { scores: GameScoreSummary[] }) {
  return <section className={styles.phase2Card} aria-labelledby="game-scores-title"><div className={styles.phase2Head}><h2 id="game-scores-title">🎮 Oyun Skorların</h2><span>En iyi skor</span></div><div className={styles.phase2GameGrid}>{scores.map((game) => <article key={game.slug} className={styles.phase2Game}><div><strong>{game.title}</strong><small>{game.bestScore === null ? "Henüz skor yok" : `${game.bestScore} puan${game.lastPlayed ? ` · ${formatResultDate(game.lastPlayed)}` : ""}`}</small></div><Link href={game.href} aria-label={`${game.title} oyna`}>Oyna</Link></article>)}</div></section>;
}

function BadgeSummary({ xpSnapshot }: { xpSnapshot: StudentXpSnapshot }) {
  const badges = getStudentXpBadges(xpSnapshot);
  const earned = badges.filter((badge) => badge.isEarned);
  return <section className={styles.badgeSummary} aria-labelledby="badge-summary-title"><div><strong id="badge-summary-title">🏅 Rozetler</strong><small>{earned.length} rozet kazandın</small></div><div className={styles.badgeSummaryIcons}>{earned.slice(0, 3).map((badge) => <span key={badge.id} title={badge.title}>🏅</span>)}</div><Link href="/ogrenci/rozetlerim">Tümünü gör</Link></section>;
}

function StudentUsageGuideCard() {
  return (
    <section className={styles.usageGuideCard} aria-labelledby="student-usage-guide-title" data-student-usage-guide>
      <a
        href="/belgeler/ogrenci-kullanim-rehberi.pdf"
        target="_blank"
        rel="noopener noreferrer"
        className={styles.usageGuideLink}
      >
        <span className={styles.usageGuideIcon} aria-hidden="true"><Icon name="bookOpen" /></span>
        <span className={styles.usageGuideCopy}>
          <strong id="student-usage-guide-title">📘 Kullanım Rehberi</strong>
          <small>Platformu nasıl kullanacağını öğren</small>
        </span>
        <Icon name="arrow" />
      </a>
    </section>
...
export function StudentPanelPreview({ authenticatedStudent, showReadingTestsCard = false, showStatisticsCard = false, showParagraphAnalyticsCard = false, xpSnapshot }: StudentPanelPreviewProps) {
  ...
  const weeklyProgress = useMemo(() => getStudentPanelWeeklyProgress(resultsState.results), [resultsState.results]);
  const achievements = useMemo(() => getStudentPanelAchievements(resultsState.results), [resultsState.results]);
  const gameScores = useMemo(() => getStudentPanelGameScores(resultsState.results), [resultsState.results]);
  ...
  return <HeroThemeProvider><main className={`${styles.preview} ${light ? styles.light : ""}`}><div className={styles.shell}><Sidebar onDemo={showToast} onAccountMenu={openAccountMenu} accountMenuOpen={panel === "profile"} streakValue={streakValue} streakNote={streakNote} xpSnapshot={safeXpSnapshot}/><div className={styles.content}><div className={styles.mobileHeader}><Brand/><button type="button" aria-label="Menüyü aç" aria-expanded={panel === "menu"} onClick={() => togglePanel("menu")}><Icon name="menu"/></button><button type="button" aria-label="Bildirimler" aria-expanded={panel === "notifications"} onClick={() => togglePanel("notifications")}><Icon name="bell"/></button></div><Header light={light} panel={panel} studentName={studentIdentity.name} classLabel={studentIdentity.classLabel} onToggleTheme={() => setTheme(light ? "dark" : "light")} onTogglePanel={togglePanel}/><div className={styles.heroGrid}><Hero studentName={studentIdentity.name} resumeTarget={resumeTarget} progressSummary={heroProgressSummary}/></div><StudentUsageGuideCard/><div className={styles.dashboardGrid}><div className={styles.mainColumn}><section className={styles.statsGrid} aria-label="İstatistikler">{visibleDashboardStats.map((stat,index) => <StatCard key={stat.label} stat={stat} index={index}/>)}</section><StudentRecommendationsCard/><WeeklyProgressCard progress={weeklyProgress} results={resultsState.results} loading={resultsLoading}/><AchievementsCard achievements={achievements} loading={resultsLoading}/><GameScoresCard scores={gameScores}/>{showParagraphAnalyticsCard ? <ParagraphAnalyticsCard /> : null}{showReadingTestsCard && <ReadingTestsCard results={resultsState.results} status={resultsState.status}/>}<RecentResults results={recentResults} loading={resultsLoading} error={resultsError}/><section className={styles.categoriesSection}><div className={styles.sectionTitle}><div><h2>🚀 Egzersiz Kategorileri</h2><p>Göz, dikkat, okuma ve hafıza becerilerini geliştir.</p></div><Link href="/egzersizler">Tüm Egzersizler <Icon name="arrow"/></Link></div><div className={styles.categoryGrid}>{categories.map((category,index) => <CategoryCard key={category.title} category={category} index={index}/>)}</div></section></div><aside className={styles.rightColumn}><LevelCard xpSnapshot={safeXpSnapshot} lastReward={lastReward} recommendation={panelRecommendation}/><BadgeSummary xpSnapshot={safeXpSnapshot}/><GameRoomJoinCard/><ReadingTest test={lastReadingTest} loading={resultsLoading}/>{showStatisticsCard && <StatisticsCard/>}<section className={styles.motivation}><div><strong>Unutma!</strong><p>Her gün küçük adımlar,<br/>büyük gelişimler getir.</p></div><span>🪐</span></section></aside></div></div></div><MobileNav onDemo={showToast} onProfile={openAccountMenu} profileOpen={panel === "profile"}/>{panel && <><button type="button" className={styles.panelBackdrop} aria-label="Açık paneli kapat" onClick={() => setPanel(null)}/>{panel === "menu" ? <MobileMenu onDemo={showToast} onClose={() => setPanel(null)} onAccountMenu={openAccountMenu} accountMenuOpen={false}/> : <DemoPopover panel={panel} studentName={studentIdentity.name} classLabel={studentIdentity.classLabel} onDemo={showToast} onClose={() => setPanel(null)} onLogout={() => void handleLogout()} isLoggingOut={isLoggingOut}/>}</>}{logoutError && <div className={styles.logoutError} role="alert">{logoutError}</div>}{toast && <div className={styles.toast} role="status" aria-live="polite">{toast}</div>}</main></HeroThemeProvider>;
}
