...
const MODE_OPTIONS: GameMode[] = ["word", "letter", "symbol", "number"];
...
export function CatchSameExerciseClient({
  ...
  const effectiveDuration =
    assignmentTask && assignmentTask.durationSeconds > 0
      ...
      : (educationProgramLaunch?.durationSeconds ?? selectedDuration);
  ...
  async function persistResult(payload: SecureExerciseResultInput) {
    ...
    setFeedback("Sonuç kaydediliyor...");
    try {
  ...
  function finishExercise() {
    ...
    setStatus("finished");
    setIsChanging(false);
    setFeedback("Egzersiz sonlandırıldı. Sonuç kaydediliyor...");
    saveResult("manual");
  }
