    ...
    <div className="grid gap-2 lg:grid-cols-5">
      ...
      <div className="grid gap-2 sm:grid-cols-3 lg:col-span-3">
        {phase === "ready" ? (
          ...
          <>
            <button type="button" className={FULLSCREEN_PRIMARY_BUTTON_CLASS} style={FULLSCREEN_TOUCH_STYLE} onClick={togglePause}>
  ...
  return (
    ...
    >
      <div className="w-full max-w-4xl">
        <article className="rounded-[26px] border border-red-100 bg-white px-4 py-5 text-left leading-9 shadow-[0_18px_50px_rgba(185,28,28,0.10)] md:px-7 md:py-6 md:leading-10">
          {round.words.map((word) => {
            const feedbackClass =
              feedback?.wordId === word.id
                ? feedback.type === "correct"
                  ? "fx-glow-green border-green-200 bg-green-100 text-green-800"
                  : "fx-blink-red border-red-300 bg-red-100 text-red-800"
                : "border-transparent bg-transparent text-slate-800 hover:border-red-100 hover:bg-red-50";
        ...
        <div className="mt-5 rounded-[24px] border border-red-200/90 bg-[linear-gradient(135deg,#fff7f7_0%,#fee2e2_100%)] px-5 py-4 text-center shadow-inner shadow-red-100/70">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-red-700">Bulunacak kelime</p>
          <p className="mt-2 text-3xl font-black text-slate-950">{activeTarget?.normalized ?? "-"}</p>
        </div>
