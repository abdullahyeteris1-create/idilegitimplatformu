...
export default async function Page({ searchParams }: { searchParams: Promise<{ educationLaunch?: string }> }) {
  ...
  const launch = await resolveEducationProgramExerciseLaunch(params.educationLaunch, "hedef-toplam");
  return <EducationProgramExerciseChrome launch={launch}><MentalArithmeticGameClient kind="target" educationProgramLaunch={launch ?? undefined} /></EducationProgramExerciseChrome>;
}
