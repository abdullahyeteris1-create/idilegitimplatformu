...
export default async function Page({ searchParams }: { searchParams: Promise<{ educationLaunch?: string }> }) {
  ...
  const launch = await resolveEducationProgramExerciseLaunch(params.educationLaunch, "hazine-kasasi");
  return <EducationProgramExerciseChrome launch={launch}><MentalArithmeticGameClient kind="vault" educationProgramLaunch={launch ?? undefined} /></EducationProgramExerciseChrome>;
}
