...
export function VaultGameClient({ educationProgramLaunch }: Props) {
  ...
  const [vaultState, setVaultState] = useState<"closed" | "open" | "correct" | "wrong">("closed");
  ...
  function prepareRound(number: number) {
    ...
    const nextQuestion = generateVaultQuestion(level, digits, mode); setQuestion(nextQuestion); setRoundNumber(number); setInputs(Array.from({ length: digits }, () => "")); setActiveInput(0); setAnswered(false); roundResolved.current = false; setVaultState("closed"); setFeedback({ good: false, title: "", text: "Şifreyi çöz ve kasayı aç!" }); setTimeLeft(time * 10); setProgress(getVaultProgress(number, roundLimit)); setSpeedBadge(time === 0 ? "⭐ Rahat mod: sabit hız bonusu" : `⚡ ${time} sn içinde ne kadar hızlı, o kadar çok puan!`); roundStartedAt.current = performance.now(); setPlaying(true); window.setTimeout(() => inputRefs.current[0]?.focus(), 0);
  ...
  function resolveAnswer(timedOut = false) {
    ...
    if (isCorrect) { const nextStreak = streak + 1; const earned = getVaultSpeedScore(level, time, timeLeft, nextStreak); setCorrect((count) => count + 1); setScore((points) => points + earned.total); setStreak(nextStreak); setBestStreak((best) => Math.max(best, nextStreak)); setVaultState("correct"); setFeedback({ good: true, title: `✅ ${earned.label}! Kasa açıldı!`, text: `+${earned.total} puan${time ? ` • ${elapsed.toFixed(1)} sn` : ""}${earned.streak ? ` • Seri +${earned.streak}` : ""}` }); setSpeedBadge(`🏅 Hız bonusu: +${earned.speed} puan`); setFloatScore(`+${earned.total} ${earned.label}`); if (scoreTimer.current) window.clearTimeout(scoreTimer.current); scoreTimer.current = window.setTimeout(() => setFloatScore(""), 1100); burst(); playSuccess(); }
    else { setWrong((count) => count + 1); setStreak(0); setVaultState("wrong"); setFeedback(timedOut ? { good: false, title: `⏰ Süre doldu! Doğru şifre: ${question.answer}`, text: "Hız bonusu kaçtı." } : { good: false, title: `❌ Yanlış şifre. Doğru cevap: ${question.answer}`, text: "Sonraki kasada daha dikkatli ol!" }); setSpeedBadge(timedOut ? "⏱️ Süre bonusu kaçtı!" : "💡 Sonraki kasada daha dikkatli ol!"); playFailure(); }
    scheduleNext(roundNumber, token.current, timedOut ? 1400 : 1500);
  }
