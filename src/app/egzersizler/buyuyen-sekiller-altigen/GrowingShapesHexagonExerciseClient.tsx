...
const SETTINGS_SELECT_CLASS = "h-11 w-full min-w-0 whitespace-nowrap rounded-xl border border-[var(--idil-border)] bg-[var(--idil-surface-strong)] px-3 pr-10 text-sm leading-normal text-[var(--idil-text)] outline-none transition focus-visible:ring-2 focus-visible:ring-red-400 disabled:cursor-not-allowed disabled:opacity-60";
...
export function GrowingShapesHexagonExerciseClient({ educationProgramLaunch }: { educationProgramLaunch?: EducationProgramExerciseLaunchProps } = {}) {
  ...
  const themedLabelClass = `${SETTINGS_LABEL_CLASS} ${theme === "dark" ? "text-slate-100" : ""}`;
  const themedSelectClass = `${SETTINGS_SELECT_CLASS} ${theme === "dark" ? "scheme-dark border-slate-500 bg-slate-900/90 text-slate-100 disabled:border-slate-500 disabled:bg-slate-800 disabled:text-slate-300 disabled:opacity-100" : "scheme-light"}`;
  const optionClass = theme === "dark" ? "bg-slate-900 text-slate-100" : "bg-white text-slate-900";
  const settingControls = <div className="grid grid-cols-1 gap-3 rounded-2xl border border-[var(--idil-border)] bg-[var(--idil-surface)] px-4 py-4 sm:grid-cols-2 md:grid-cols-4 xl:grid-cols-[1.1fr_1fr_1fr_1.1fr_1fr_0.8fr_0.9fr_0.75fr] xl:items-end xl:gap-3 sm:px-5">
    <div className="min-w-0"><label htmlFor="growing-shapes-duration" className={themedLabelClass}>Egzersiz Süresi</label><select id="growing-shapes-duration" className={themedSelectClass} disabled={settingsLocked} value={durationSeconds} onChange={(event) => setDurationSeconds(Number(event.target.value) as DurationSeconds)}>{DURATIONS.map((value) => <option className={optionClass} key={value} value={value}>{formatTime(value)}</option>)}</select></div>
    <div className="min-w-0"><label htmlFor="growing-shapes-speed-mode" className={themedLabelClass}>Hız Modu</label><select id="growing-shapes-speed-mode" className={themedSelectClass} disabled={settingsLocked} value={speedMode} onChange={(event) => setSpeedMode(event.target.value as SpeedMode)}>{SPEED_MODES.map((value) => <option className={optionClass} key={value} value={value}>{value === "fixed" ? "Sabit Hız" : "Değişken Hız"}</option>)}</select></div>
    <div className="min-w-0"><label htmlFor="growing-shapes-jump-duration" className={themedLabelClass}>Sıçrama Süresi</label><select id="growing-shapes-jump-duration" className={themedSelectClass} disabled={settingsLocked} value={jumpDurationMs} onChange={(event) => setJumpDurationMs(Number(event.target.value))}>{JUMP_DURATIONS.map((value) => <option className={optionClass} key={value} value={value}>{value} ms</option>)}</select></div>
    <div className="min-w-0"><label htmlFor="growing-shapes-jump-end" className={themedLabelClass}>Sıçrama Bitişi</label><select id="growing-shapes-jump-end" title="Yalnız değişken hız modunda kullanılır." className={themedSelectClass} disabled={settingsLocked || speedMode === "fixed"} value={jumpEndDurationMs} onChange={(event) => setJumpEndDurationMs(Number(event.target.value))}>{JUMP_END_DURATIONS.map((value) => <option className={optionClass} key={value} value={value}>{value} ms</option>)}</select></div>
    <div className="min-w-0"><label htmlFor="growing-shapes-clear-mode" className={themedLabelClass}>Silme Modu</label><select id="growing-shapes-clear-mode" className={themedSelectClass} disabled={settingsLocked} value={clearMode} onChange={(event) => setClearMode(event.target.value as ClearMode)}>{CLEAR_MODES.map((value) => <option className={optionClass} key={value} value={value}>{value === "with-clearing" ? "Silerek" : "Silmeden"}</option>)}</select></div>
    <div className="min-w-0"><span className={themedLabelClass}>Metronom</span><label htmlFor="growing-shapes-metronome" className="flex h-11 min-w-0 items-center"><input id="growing-shapes-metronome" type="checkbox" aria-label="Metronom seslerini aç" disabled={settingsLocked} checked={showMetronome} onChange={(event) => setShowMetronome(event.target.checked)} className="h-5 w-5 accent-red-500 border border-slate-400 bg-[var(--idil-surface-strong)] disabled:opacity-100" /></label></div>
    <div className="min-w-0"><span className={themedLabelClass}>Odak Noktası</span><label htmlFor="growing-shapes-focus-point" className="flex h-11 min-w-0 items-center"><input id="growing-shapes-focus-point" type="checkbox" aria-label="Odak noktasını göster" disabled={settingsLocked} checked={showFocusPoint} onChange={(event) => setShowFocusPoint(event.target.checked)} className="h-5 w-5 accent-red-500 border border-slate-400 bg-[var(--idil-surface-strong)] disabled:opacity-100" /></label></div>
    <div className="min-w-0"><span className={themedLabelClass}>Köşeler</span><label htmlFor="growing-shapes-corners" className="flex h-11 min-w-0 items-center"><input id="growing-shapes-corners" type="checkbox" aria-label="Köşeleri işaretle" disabled={settingsLocked} checked={showCorners} onChange={(event) => setShowCorners(event.target.checked)} className="h-5 w-5 accent-red-500 border border-slate-400 bg-[var(--idil-surface-strong)] disabled:opacity-100" /></label></div>
  </div>;
