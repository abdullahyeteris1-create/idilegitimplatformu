  ...
  const footerControls = (
    <div className="grid gap-2 lg:grid-cols-9">
      ...
      <label className="flex min-w-0 flex-col gap-1 lg:col-span-2">
        <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Gorsel</span>
        <select value={selectedImageId} onChange={(event) => handleImageChange(event.target.value)} className={FULLSCREEN_SELECT_CLASS}>
      ...
      <label className="flex min-w-0 flex-col gap-1">
        <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Yardim</span>
        <select value={helpMode ? "on" : "off"} onChange={(event) => setHelpMode(event.target.value === "on")} className={FULLSCREEN_SELECT_CLASS}>
        ...
        {phase === "ready" ? (
          ...
          <>
            {phase === "paused" ? (
              ...
              <button type="button" className={FULLSCREEN_SECONDARY_BUTTON_CLASS} style={FULLSCREEN_TOUCH_STYLE} onClick={handlePause} disabled={phase !== "playing" || isResolving}>
                Duraklat
              </button>
            )}
        ...
        <div className="mt-5 grid w-full max-w-4xl gap-4 md:grid-cols-[minmax(0,1fr)_260px] md:items-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-left md:grid-cols-1">
            <article className="rounded-2xl border border-red-100 bg-red-50 p-3">
              <p className="text-xs font-bold text-slate-500">Grid</p>
              <p className="mt-1 text-2xl font-black text-red-700">{startGrid.cols}x{startGrid.rows}</p>
            </article>
            <article className="rounded-2xl border border-red-100 bg-red-50 p-3">
              <p className="text-xs font-bold text-slate-500">Parca</p>
              <p className="mt-1 text-2xl font-black text-red-700">{startGrid.rows * startGrid.cols}</p>
            </article>
            <article className="rounded-2xl border border-red-100 bg-red-50 p-3">
              <p className="text-xs font-bold text-slate-500">Gorsel</p>
              <p className="mt-1 text-lg font-black text-red-700">{selectedImage.title}</p>
            </article>
            <article className="rounded-2xl border border-red-100 bg-red-50 p-3">
    ...
    >
      <div className="flex w-full flex-1 flex-col gap-3">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-red-100 bg-red-50 px-4 py-3 text-left">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-red-700">YONERGE</p>
            <p className="mt-1 text-xl font-black text-slate-950 md:text-2xl">Iki kare secerek yerlerini degistir.</p>
            <p className="mt-1 text-sm font-semibold text-slate-500">Resmi tamamlamak icin karisan kareleri dogru siraya getir.</p>
          </div>
          {helpMode ? (
            <div className="flex items-center gap-3 rounded-2xl border border-white/80 bg-white px-3 py-2 text-left shadow-sm">
              <div
              ...
              />
              <div>
                <p className="text-xs font-bold text-slate-500">Orijinal Gorsel</p>
                <p className="text-sm font-black text-red-700">{selectedImage.title}</p>
              </div>
            </div>
          ) : null}
        </div>
