...
export default function GozCalismasiPage() {
  ...
  return (
    <ExerciseStage
      ...
      footer={<div className="flex flex-wrap justify-center gap-2">{!isRunning ? <button type="button" onClick={startExercise} className="min-h-11 rounded-xl bg-emerald-600 px-5 font-bold text-white">Başlat</button> : <button type="button" onClick={stopExercise} className="min-h-11 rounded-xl bg-red-600 px-5 font-bold text-white">Durdur</button>}<button type="button" onClick={resetExercise} className="min-h-11 rounded-xl border border-slate-300 bg-white px-5 font-bold">Sıfırla</button></div>}
    >
    <main className="h-full min-h-0 min-w-0 w-full max-w-full overflow-hidden bg-white text-slate-900">
      <section className="mx-auto flex h-full min-h-0 w-full max-w-6xl flex-col">
        <section className="relative min-h-0 flex-1 overflow-hidden rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_center,_rgba(239,68,68,0.05),_rgba(255,255,255,1)_55%,_rgba(248,250,252,1))] shadow-sm">
