...
export default function KelimeTahminPage() {
  ...
  function submitGuess() {
    ...
    if (guess.length !== wordLength) {
      setMessage(`Lütfen ${wordLength} harfli bir kelime yaz.`);
      return;
    }
    ...
    if (nextGuesses.length >= MAX_ATTEMPTS) {
      setStatus("lost");
      setMessage(`Kaybettin. Doğru kelime: ${answer}`);
      return;
    }
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900">
      <div className="mx-auto max-w-4xl">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          ...
          <div className="mb-5 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
            <div className="text-center text-sm font-semibold sm:text-left">
              {message}
            </div>
          ...
          <div className="space-y-2">
            {rows.map((row, rowIndex) => {
              ...
              const states = isSubmitted
                ? evaluateGuess(row, answer, wordLength)
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <input
              value={currentGuess}
              onChange={(event) =>
                setCurrentGuess(normalizeInput(event.target.value, wordLength))
              }
              onKeyDown={(event) => {
              ...
              className="min-h-12 flex-1 rounded-2xl border border-slate-300 px-4 text-center text-lg font-bold uppercase outline-none focus:border-red-400"
            />

            <button
              onClick={submitGuess}
              disabled={status !== "playing"}
              className="min-h-12 rounded-2xl bg-red-600 px-6 font-bold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              Tahmin Et
            </button>
          ...
          <div className="mt-5 grid gap-3 rounded-2xl bg-slate-100 p-4 text-sm text-slate-700 sm:grid-cols-3">
            <div>
              <span className="font-bold">Kelime Uzunluğu:</span> {wordLength}
            </div>
            <div>
              <span className="font-bold">Deneme:</span> {guesses.length}/
              {MAX_ATTEMPTS}
            </div>
            <div>
              ...
              <div key={rowIndex} className="flex justify-center gap-1.5">
                ...
                {rowIndex === 2 && (
                  <button
                    onClick={submitGuess}
                    disabled={status !== "playing"}
                    className="rounded-lg bg-red-600 px-3 text-xs font-bold text-white hover:bg-red-700 disabled:bg-slate-300"
                  >
                    Gir
                  </button>
                )}
          ...
          <div className="mt-6 rounded-2xl bg-slate-100 p-4 text-sm text-slate-600">
            <p>
              <strong>Yeşil:</strong> Harf doğru yerde.
            </p>
            <p>
              <strong>Sarı:</strong> Harf kelimede var ama yeri yanlış.
            </p>
            <p>
