...
import { publishExam } from "@/lib/paragraph-exams/repository";
...
export async function POST(request: NextRequest, context: { params: Promise<{ examId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, exam: await publishExam(examId) });
  } catch (error) { return repositoryErrorResponse(error, "Sınav yayınlanamadı."); }
}
