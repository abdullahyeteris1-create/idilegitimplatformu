...
import { duplicateExam } from "@/lib/paragraph-exams/repository";
...
export async function POST(request: NextRequest, context: { params: Promise<{ examId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, exam: await duplicateExam(examId, createdBy) }, { status: 201 });
  } catch (error) { return repositoryErrorResponse(error, "Sınav kopyalanamadı."); }
}
