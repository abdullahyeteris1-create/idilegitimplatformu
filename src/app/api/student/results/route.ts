...
const DETAIL_SCHEMAS: Record<string, Record<string, DetailRule>> = {
  ...
  "one-minute-oral-reading": {
    textId: { type: "string", maxLength: 128 },
    textTitle: { type: "string", maxLength: 160 },
    grade: { type: "integer", min: 1, max: 8 },
    wordCount: { type: "integer", min: 0, max: 1_000_000 },
    wordsRead: { type: "integer", min: 0, max: 1_000_000 },
    readingErrors: { type: "integer", min: 0, max: 1_000_000 },
    correctWords: { type: "integer", min: 0, max: 1_000_000 },
    totalQuestions: { type: "integer", min: 0, max: 100 },
    comprehensionCorrect: { type: "integer", min: 0, max: 100 },
    comprehensionWrong: { type: "integer", min: 0, max: 100 },
    comprehensionScore: { type: "number", min: 0, max: 100 },
    completedAt: { type: "string", maxLength: 40 },
  },
  "listen-then-read": {
    passageId: { type: "string", maxLength: 128 },
    passageTitle: { type: "string", maxLength: 160 },
    grade: { type: "integer", min: 1, max: 4 },
    listeningCompleted: { type: "boolean" },
    readingCompleted: { type: "boolean" },
    comprehensionCorrect: { type: "integer", min: 0, max: 100 },
    comprehensionTotal: { type: "integer", min: 0, max: 100 },
    comprehensionWrong: { type: "integer", min: 0, max: 100 },
    comprehensionScore: { type: "number", min: 0, max: 100 },
  ...
  for (const [key, raw] of Object.entries(value)) {
    ...
    if (rule.type === "array") {
      ...
      const answers = raw.map((item) => {
        ...
        if (item.selectedIndex !== undefined && (typeof item.selectedIndex !== "number" || !Number.isInteger(item.selectedIndex) || item.selectedIndex < 0 || item.selectedIndex > 4)) return null;
        return { questionId: item.questionId, ...(item.category !== undefined ? { category: item.category } : {}), correct: item.correct, responseTimeMs: item.responseTimeMs, ...(item.selectedIndex !== undefined ? { selectedIndex: item.selectedIndex } : {}) };
      });
