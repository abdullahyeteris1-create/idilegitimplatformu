-- ============================================================================
-- Fix: assign_education_program_template_v1 - bayat egzersiz beyaz listesi
-- ============================================================================
-- Bug: yayinlanmis, aktif, gecerli gun/gorev sayisina sahip bir sablon
-- atanirken RPC 'STUDENT_EDUCATION_TEMPLATE_INVALID' donduruyordu ("Sablon
-- eksik veya gecersiz."), cunku bu fonksiyonun t.exercise_slug not in (...)
-- beyaz listesi (20260725180000) yalniz ilk yazildigi andaki 9 egzersizi
-- iceriyordu. Sonradan src/lib/education-programs/exerciseCatalog.ts'e 5 yeni
-- egzersiz eklendi (blok-okuma, golgeleme, gruplama-calismasi, anlama-testi,
-- okuma-hizi-testi) ama bu RPC hic guncellenmedi - sablon olusturma/duzenleme/
-- yayinlama JS tarafinda guncel katalogla calistigi icin sorunsuz geciyor,
-- ama atama anindaki bu SQL kontrolu hala eski listeyi kullandigindan bu 5
-- egzersizden birini iceren gunleri "gecersiz gorev" sayip reddediyordu.
--
-- Bu migration YALNIZCA satir 369-379'daki (orijinal migration numarasi)
-- whitelist'i genisletir - fonksiyonun imzasi, guvenlik modeli ve diger tum
-- guard'lari (gun sayisi, gun basina 5 gorev, duration/settings kontrolleri)
-- degismeden aynen korunur.
-- ============================================================================
...
begin
  ...
  -- Egzersiz beyaz listesi src/lib/education-programs/exerciseCatalog.ts
  -- (EDUCATION_PROGRAM_EXERCISE_CATALOG) ile senkron tutulmalidir - orada
  -- yeni bir egzersiz eklendiginde burasi da guncellenmelidir.
  if exists (
    select 1
    from public.education_program_template_days d
...
comment on function public.assign_education_program_template_v1(
  ...
  'Atomically assigns one published active education program template as an immutable student snapshot. Exercise whitelist synced with exerciseCatalog.ts (2026-07-28: added blok-okuma, golgeleme, gruplama-calismasi, anlama-testi, okuma-hizi-testi).';
