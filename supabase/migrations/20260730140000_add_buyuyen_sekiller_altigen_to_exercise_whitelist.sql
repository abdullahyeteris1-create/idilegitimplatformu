-- Forward whitelist migration for the Education Program exercise catalog.
-- This migration is intentionally not executed by the application task.
...
begin
  ...
  then raise exception using errcode = 'P0001', message = 'STUDENT_EDUCATION_INPUT_INVALID'; end if;
  ...
  if exists (select 1 from public.education_program_template_days d join public.education_program_template_tasks t on t.template_day_id = d.id where d.template_id = p_template_id and (t.exercise_slug is null or t.exercise_slug not in (
    'kare-gorme-alani', 'ayni-olani-yakala', 'benzer-kelimeler', 'kelime-bulma', 'goz-egzersizleri-kolonlar', 'takistoskop', 'harf-rakam-sayma', 'hafiza-gelistirme', 'kart-eslestirme', 'blok-okuma', 'cift-tarafli-odak', 'goz-kaslari', '13-nokta-emoji-takip', 'buyuyen-sekiller-altigen', 'golgeleme', 'gruplama-calismasi', 'anlama-testi', 'okuma-hizi-testi'
...
comment on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) is 'Atomically assigns one published active education program template as an immutable student snapshot. Exercise whitelist synced with exerciseCatalog.ts; 2026-07-30 added buyuyen-sekiller-altigen.';
