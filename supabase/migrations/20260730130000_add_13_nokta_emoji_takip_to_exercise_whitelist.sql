-- ============================================================================
-- Add 13-nokta-emoji-takip to assign_education_program_template_v1 whitelist
-- ============================================================================
-- The exercise is added to the application Education Program catalog in the
-- same release. Keep this forward migration aligned with that catalog.
...
begin
  ...
  if exists (
    select 1 from public.student_education_programs
  ...
  select count(*)::integer, count(distinct day_number)::integer,
    min(day_number)::integer, max(day_number)::integer
  into v_day_count, v_distinct_day_count, v_first_day, v_last_day
  ...
  if exists (
    ...
    left join public.education_program_template_tasks t on t.template_day_id = d.id
    ...
    where d.template_id = p_template_id
      and (
        ...
        or t.exercise_slug not in (
          'kare-gorme-alani', 'ayni-olani-yakala', 'benzer-kelimeler',
          'kelime-bulma', 'goz-egzersizleri-kolonlar', 'takistoskop',
          'harf-rakam-sayma', 'hafiza-gelistirme', 'kart-eslestirme',
          'blok-okuma', 'cift-tarafli-odak', 'goz-kaslari',
          '13-nokta-emoji-takip', 'golgeleme', 'gruplama-calismasi',
          'anlama-testi', 'okuma-hizi-testi'
        )
  ...
  insert into public.student_education_programs (
    student_id, source_template_id, source_template_version, source_template_name,
    visible_name, student_message, admin_note, status, current_day_number,
    completed_days, total_days, assigned_by
  )
  values (
    p_student_id, v_template.id, v_template.version, v_template.name,
    v_visible_name, v_student_message, v_admin_note, 'active', 1, 0,
    v_template.day_count, v_assigned_by
  ) returning id into v_program_id;

  insert into public.student_education_program_days (
    program_id, day_number, title, description, status, available_at
  )
  select v_program_id, d.day_number, d.title, d.description,
  ...
  insert into public.student_education_program_tasks (
    program_id, program_day_id, student_id, day_number, order_number,
    exercise_slug, exercise_title, result_exercise_type, starting_level,
    duration_seconds, settings_schema_version, settings, status
  )
  select v_program_id, snapshot_day.id, p_student_id, template_day.day_number,
    template_task.order_number, template_task.exercise_slug,
    template_task.exercise_title, template_task.result_exercise_type,
    template_task.starting_level, template_task.duration_seconds,
    template_task.settings_schema_version, template_task.settings,
    case when template_day.day_number = 1 and template_task.order_number = 1
  ...
  if (select count(*) from public.student_education_program_days where program_id = v_program_id) <> v_template.day_count
    or (select count(*) from public.student_education_program_tasks where program_id = v_program_id) <> v_template.day_count * 5
  then
...
revoke all on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) from public;
revoke all on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) from anon;
revoke all on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) from authenticated;
grant execute on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) to service_role;

comment on function public.assign_education_program_template_v1(uuid, uuid, text, text, text, text) is
  'Atomically assigns one published active education program template as an immutable student snapshot. Exercise whitelist synced with exerciseCatalog.ts; 2026-07-30 added 13-nokta-emoji-takip.';
