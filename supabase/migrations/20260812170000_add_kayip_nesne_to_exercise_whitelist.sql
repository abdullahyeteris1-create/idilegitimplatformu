-- Add Kayıp Nesne to the education-program assignment whitelist.
...
begin
  ...
  if exists (select 1 from public.education_program_template_days d join public.education_program_template_tasks t on t.template_day_id = d.id where d.template_id = p_template_id and (t.exercise_slug is null or t.exercise_slug not in (
    'kare-gorme-alani', 'ayni-olani-yakala', 'benzer-kelimeler', 'kelime-bulma', 'goz-egzersizleri-kolonlar', 'takistoskop', 'harf-rakam-sayma', 'hafiza-gelistirme', 'kart-eslestirme', 'blok-okuma', 'cift-tarafli-odak', 'goz-kaslari', '13-nokta-emoji-takip', 'buyuyen-sekiller-altigen', 'kelime-yarisi', 'golgeleme', 'gruplama-calismasi', 'anlama-testi', 'okuma-hizi-testi', 'tatli-dukkani', 'kayip-nesne'
